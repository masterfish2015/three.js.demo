...
function _draw_pcb_shape_arc (arc) {
  ...
  let x = parseFloat(arc['LOCATION.X']),
    ...
    start_angle = parseFloat(arc['STARTANGLE']) * Math.PI / 180.0,
    end_angle = parseFloat(arc['ENDANGLE']) * Math.PI / 180.0,
    width = parseFloat(arc['WIDTH'])
  let cos_start_ang = Math.cos(start_angle),
    cos_end_ang = Math.cos(end_angle),
    sin_start_ang = Math.sin(start_angle),
    sin_end_ang = Math.sin(end_angle)

  shape.moveTo((r - width / 2) * cos_start_ang, (r - width / 2) * sin_start_ang)
  shape.lineTo((r + width / 2) * cos_start_ang, (r + width / 2) * sin_start_ang)
  shape.absarc(0, 0, r + width / 2, start_angle, end_angle, true)
  shape.moveTo((r + width / 2) * cos_end_ang, (r + width / 2) * sin_end_ang)
  shape.lineTo((r - width / 2) * cos_end_ang, (r - width / 2) * sin_end_ang)
  shape.absarc(0, 0, r - width / 2, end_angle, start_angle, true)
